...
export default function Home() {
  ...
  return (
    <div className="container mx-auto px-4 mt-24 space-y-8">
      {/* Welcome Banner */}
      <div className="hover:scale-105 hover:shadow-xl bg-gradient-to-b from-black to-gray-500 p-6 rounded-xl shadow text-center">
      ...
      {/* Current Balance */}
      <div className="hover:scale-105 hover:shadow-xl bg-white focus:scale-105 p-6 rounded-xl shadow border flex flex-col items-center">
        <h2 className="text-2xl font-bold text-black">Current Balance</h2>
        <p
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col rounded-xl border bg-white shadow overflow-hidden">
          ...
          <div className="px-6 py-4 flex flex-row items-center justify-between border-b bg-white">
            <h4 className="text-lg font-medium">Recent Expenses & Income</h4>
            <button
        ...
        <div className="flex flex-col rounded-xl border bg-white shadow overflow-hidden">
          ...
          <div  className="px-6 py-4  space-y-4">
            {groups.length > 0 ? (
              groups.map((group) => {
                // ✅ FIX: lookup using email
                const bal = balances[group._id]?.[user.email] ?? null;
                ...
                return (
                  ...
                  navigate(`/dashboard/groups/${group._id}/expenses`)
                    ...
                    className="hover:bg-gray-100 flex items-center justify-between py-3 border-b border-border last:border-b-0 hover:bg-muted/50 rounded-lg px-2 -mx-2 cursor-pointer transition-colors"
                  >
                    {/* Left */}
                    <div className="flex  items-center space-x-3">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 mt-2 hidden group-hover:block z-10 w-max bg-gray-900 text-white text-xs rounded-lg shadow-lg px-2 py-1">
        {member}
                            ...
                            {group.members.length > 3 && (
                              <span
                                title={group.members.slice(3).join(", ")} // ✅ hover shows remaining emails
